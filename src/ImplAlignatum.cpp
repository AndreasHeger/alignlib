				   ...
				   const bool unaligned_chars) 
{
  ...
  mFrom = getResidueNumbernext(row_from-1); 
  ...
  // substitute new characters for aligned positions:
  {
    ...
    for (; it != it_end; ++it) 
      new_representation[it->mCol-1] = mRepresentation[it->mRow-1];
  }
...
void ImplAlignatum::setRepresentation( std::string & representation, 
  ...
  // set first residue number to 1 if not given
  if (first_res < 1) 
    first_res = 1;
