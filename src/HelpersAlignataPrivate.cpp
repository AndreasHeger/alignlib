/*
  ...
  $Id: HelpersAlignataPrivate.cpp,v 1.3 2004/06/02 12:11:37 aheger Exp $
...
namespace alignlib {
  ...
  // a class that calculates the match-score between a row and column.
	...
	Match( const Alignandum * row, const Alignandum * col, const SubstitutionMatrix * matrix = NULL) {
	    ...
	    if (!row->isPrepared() )
		throw AlignException( "Row has not been prepared in HelpersAlignataPrivate::Match!");

	    if (!col->isPrepared() ) 
		throw AlignException( "Col has not been prepared in HelpersAlignataPrivate::Match!");
	}
