...
//-------------------------------------------------------------------------------------------------------------------------------
ImplRegularizorDirichlet::ImplRegularizorDirichlet (const ImplRegularizorDirichlet & src ) : mFadeCutoff(src.mFadeCutoff) {
}
...
void ImplRegularizorDirichlet::fillColumn(  Frequency * frequencies, 
  ...
  if (xtotal > 0)
    for (i = 0; i < PROFILEWIDTH; i++)
      frequencies[i] = (Frequency)(Xi[i] / xtotal);
}
						...
						const Position length ) const 
{
  debug_func_cerr(5);
	...
	if (ntotal < mFadeCutoff ) {
	    ...
	    fillColumn( frequencies[column], beta_differences, n, ntotal );
