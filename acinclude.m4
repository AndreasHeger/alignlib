...
dnl #########################################################################
dnl Check command line option --enable-purify
AC_DEFUN([adl_ENABLE_PURIFY],
[
AC_PROVIDE([$0])
AC_REQUIRE_CPP
 
ac_cv_enable_purify=no
 
dnl command line options and --help descriptions
AC_ARG_ENABLE( purify,
[  --enable-purify		compile the code using purify (if available) \[default=no\] ],
ac_cv_enable_purify=$enableval,	
ac_cv_enable_purify=no	
)

if test x"$ac_cv_enable_purify" = xyes; then
	AC_MSG_CHECKING("using purify\n")
	CXX="purify $CXX"			# use "", because CXX is used as a parameter for libtool 
						# and then purify and cxx would get separated, resulting in an error
fi
]
)

dnl #########################################################################
dnl Check command line option --enable-TCO
AC_DEFUN([AC_ENABLE_TCO],
[
AC_PROVIDE([$0])
...
dnl command line options and --help descriptions
AC_ARG_ENABLE( tco,
[  --enable-tco		compile with option TCO \[default=no\] ],
ac_cv_enable_tco=$enableval,	
ac_cv_enable_tco=no	
)

if test x"$ac_cv_enable_tco" = xyes; then
	CXXFLAGS="$CXXFLAGS -DTCO"
	AC_SUBST(CXXFLAGS)
	AC_MSG_CHECKING("Compile with option TCO")
fi
]
)
...
dnl #########################################################################
dnl Check for SWIG: 
dnl Makefile, executable and runtime libraries
AC_DEFUN([AC_PACKAGE_SWIG],
[
AC_PROVIDE([$0])
...
dnl Check, if swig is in executable path
AC_CHECK_PROG(SWIG, swig, swig)

dnl command line options and --help descriptions
AC_ARG_WITH(swig-dir,
    [  --with-swig-dir           where the root of swig is installed ($SWIG_LIB)],
    [  ac_cv_with_swig_dir="$withval" ]
)
AC_ARG_WITH(swig-lib,
    [  --with-swig-lib           where the swig library is installed. (/usr/local/lib)],
    [  ac_cv_with_swig_lib="$withval" ]
)

# adl_SEARCH_LIB_PATH_NOFUNC_NOLIBS(
#   	[
# 	#include "Python.h"
#   	void SWIG_MakePtr(char *, void *, char *); 	
#   	],
#   	[
#   	char * x;
#   	void * y;
...
#   	], 
#   	swigpy,
#   	[  $ac_cv_with_swig_dir/lib \
#   	   /usr/local/lib \
#       	   /usr/lib/swig \
#   	   /usr/local/lib/swig \
#       	   /usr/local/swig/lib \
#       	   /usr/local/swig/lib/swig \
#       	   /usr/share/swig/lib \
#       	   /usr/share/lib/swig \
#   	],
#   	[swig_libdir],
#   	AC_MSG_ERROR([Can\'t find libswig]),
#   	-lm -shared $PYTHON_INCLUDES $PYTHON_INCLUDES/config 
# )

dnl option -shared removed before -lm, results in objects not found

dnl Substitute some variables for use with SWIG
swig_includedir="$SWIG_CFLAGS"
AC_SUBST(swig_includedir)
dnl Substitute some variables for use with SWIG
AC_SUBST(swig_libdir)
]
)

dnl adl_WITH_PYTHON ([ACTION-IF-FOUND], [ACTION-IF-NOT-FOUND])
dnl build python support on users request
 
AC_DEFUN([adl_WITH_PYTHON],
 [AC_ARG_WITH([python],
    [  --with-python           build Python interface module],[ PYBIN="$withval"], [PYBIN=])
...
dnl #endif
dnl for C++ use dmallocxx instead of dmalloc
...
AC_DEFUN([adl_WITH_DMALLOC],
 [AC_ARG_WITH([dmalloc],
    [  --with-dmalloc,           link with dmalloc (a malloc() debugger)])
